...
impl ResultsPanel {
    ...
    pub(super) fn confirm_insert(&mut self, cx: &mut Context<Self>) {
        ...
        let table_q = quote_ident(&table, cx);
        let cols_q: Vec<String> = col_names.iter().map(|c| quote_ident(c, cx)).collect();
        ...
        let pk_conditions: Vec<String> = self
            ...
            .filter_map(|(col, input)| {
                ...
                let col_q = quote_ident(&col.name, cx);
                match val {
        ...
        let inverse_sql = if pk_conditions.is_empty() {
            ...
            format!("DELETE FROM {} WHERE {};", table_q, pk_conditions.join(" AND "))
        };
    ...
    fn apply_edit(&mut self, cx: &mut Context<Self>) {
        ...
        let pk_columns: Vec<&ColumnInfo> = schema.columns.iter().filter(|c| c.is_primary_key).collect();
        ...
        for ((col, old), new) in self.insert_columns.iter().zip(old_vals.iter()).zip(new_vals.iter()) {
            let col_q = quote_ident(&col.name, cx);
            if col.is_primary_key {
                match new {
                    Some(v) => pk_conditions.push(format!("{} = {}", col_q, quote_string_literal(v))),
    ...
    pub(super) fn stage_edit(
        ...
        let kind = if label.starts_with("INSERT") { WriteKind::Insert }
            else if label.starts_with("UPDATE") { WriteKind::Update }
            else if label.starts_with("DELETE") { WriteKind::Delete }
            else { WriteKind::OtherDdl };
    ...
    pub fn apply_all_pending(&mut self, cx: &mut Context<Self>) {
        let edits: Vec<(String, String, String)> = cx.update_global::<AppState, _>(|state, _cx| {
            match state.active_session_mut() {
                Some(s) => {
                    let records: Vec<(String, String, String)> = s.pending_edits.iter().map(|e| {
                        (e.sql.clone(), e.inverse_sql.clone(), e.label.clone())
                    }).collect();
        ...
        for (forward, inverse, label) in edits {
            ...
            crate::state::execute_raw_query(forward, cx);
        }
    ...
    pub(super) fn delete_row(&mut self, cx: &mut Context<Self>) {
        ...
        let all_present: bool = schema
            .columns
            .iter()
            .all(|col| table_entity.read(cx).delegate().cell_named(row_ix, &col.name).is_some());
        let inverse_sql = if all_present {
            let cols_q: Vec<String> = schema.columns.iter().map(|c| crate::state::quote_ident(&c.name, cx)).collect();
        ...
        let cells: Vec<CellDiff> = schema
            ...
            .filter_map(|col| {
                table_entity
                    ...
                    .map(|c| CellDiff {
                        column: col.name.clone(),
                        old_value: if c.is_null { None } else { Some(c.value.clone()) },
                        new_value: None,
                    })
            })
            .collect();
