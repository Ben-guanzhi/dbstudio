...
use sqlx::{Arguments, Column, Executor, MySqlPool, Row, TypeInfo, ValueRef};
use std::sync::RwLock;
...
impl MySqlConnection {
    pub fn new(pool: MySqlPool) -> Self {
        Self {
            ...
            connect: RwLock::new(ConnectionConfig::new(
                dbstudio_core::models::DatabaseType::MySQL,
                "unused".to_string(),
            )),
    ...
    async fn create_pool(config: &ConnectionConfig, password: &str) -> Result<MySqlPool> {
    ...
    /// The current pool. `.clone()` is cheap (pool handles are `Arc`s).
    fn current_pool(&self) -> Result<MySqlPool> {
        self.pool
            .read()
            .map(|g| g.clone())
            .map_err(|_| anyhow::anyhow!("MySQL pool lock poisoned"))
    }
    ...
    pub async fn execute(&self, sql: &str) -> Result<SqlResult> {
        ...
        if is_select_query(&sql, &["SHOW", "DESCRIBE", "EXPLAIN"]) {
            ...
            return Ok(build_query_result_from_rows(rows, sql, elapsed, extract_mysql_cell));
        }
    ...
    pub async fn execute_parameterized(&self, sql: &str, params: &[&str]) -> Result<SqlResult> {
        ...
        Ok(build_query_result_from_rows(rows, sql_owned, elapsed, extract_mysql_cell))
    }
    ...
    ///
    /// MySQL has no `USE database` that is safe against a pool: a `USE` only
    /// changes the single pooled connection it runs on, while the others keep
    /// the previous database. The new pool is created before the old one is
    /// dropped, so a failed switch leaves the connection intact.
    pub async fn switch_database(&self, database: &str) -> Result<()> {
        ...
        let config = self
            ...
            .read()
            .map(|g| g.clone())
            .map_err(|_| anyhow::anyhow!("MySQL config lock poisoned"))?;
        let password = self
            .password
            .read()
            .map(|g| g.clone())
            .map_err(|_| anyhow::anyhow!("MySQL password lock poisoned"))?;
...
fn extract_mysql_cell(row: &sqlx::mysql::MySqlRow, index: usize) -> ResultCell {
